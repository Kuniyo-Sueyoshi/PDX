...
# Load External data of KIRC PDXs (EGAD00001003895)
load("../../data/External_PDX/EGAD00001003895/gene.exp.hg.EGAD00001003895.rda")
d.hg.ex1 <- gene.exp.hg.EGAD00001003895$counts # length-scaled Count-estimates of Cancer component
load("../../data/External_PDX/EGAD00001003895/gene.exp.mm.EGAD00001003895.rda")
d.mm.ex1 <- gene.exp.mm.EGAD00001003895$counts  # ength-scaled Count-estimates of Stroma component

sm <- read_tsv("../../data/External_PDX/EGAD00001003895/samples_KIRC.TG.tsv") #%>% column_to_rownames("GeneSymbol")

if(all(colnames(d.hg.ex1) == colnames(d.mm.ex1))){
    ids <- c()
    for (egf in colnames(d.hg.ex1)){
        which <- grep(pattern = egf, x = sm$File.ID)
        ids <- c(ids, sm$Internal.Sample.ID[which])
    }
    sm.ex <- data.frame(str_split(ids, pattern = "_"))
    pass.ex1 <- gsub("[a-c]$", "", gsub("TG", "", sm.ex[2,]))
    NXIDs.ex1 <- sm.ex[1,]
    colnames(d.hg.ex1) <- paste("KIRC.Ex", NXIDs.ex1, pass.ex1, sep = "_")
    colnames(d.mm.ex1) <- paste("KIRC.Ex", NXIDs.ex1, pass.ex1, sep = "_")
}


# Load External data of KIRC PDXs (EGAD00001004799)
load("../../data/External_PDX/EGAD00001004799/gene.exp.hg.EGAD00001004799.rda")
d.hg.ex2 <- gene.exp.hg.EGAD00001004799$counts # length-scaled Count-estimates of Cancer component
load("../../data/External_PDX/EGAD00001004799/gene.exp.mm.EGAD00001004799.rda")
d.mm.ex2 <- gene.exp.mm.EGAD00001004799$counts  # ength-scaled Count-estimates of Stroma component

# annotation
# hg
sm.ex <- data.frame(str_split(colnames(d.hg.ex2), pattern = "_"))
pass.ex2 <- gsub("[a-c]$", "", gsub("TG", "", sm.ex[3,]))
NXIDs.ex2 <- gsub("^c[1-8]", "", gsub("TG", "", sm.ex[3,]))
colnames(d.hg.ex2) <- paste("KIRC.Ex", NXIDs.ex2, pass.ex2, sep = "_")
# mm
colnames(d.mm.ex2) <- paste("KIRC.Ex", NXIDs.ex2, pass.ex2, sep = "_")


data.frame(d.hg.ex1) %>% rownames_to_column("Gene") -> d.tmp1
data.frame(d.hg.ex2) %>% rownames_to_column("Gene") -> d.tmp2
d.hg.ex <- merge(d.tmp1, d.tmp2, by = "Gene") %>% column_to_rownames("Gene")

data.frame(d.mm.ex1) %>% rownames_to_column("Gene") -> d.tmp1
data.frame(d.mm.ex2) %>% rownames_to_column("Gene") -> d.tmp2
d.mm.ex <- merge(d.tmp1, d.tmp2, by = "Gene") %>% column_to_rownames("Gene")


if (all(colnames(d.hg.ex) == colnames(d.mm.ex))){
    sm.ex <- data.frame(str_split(colnames(d.hg.ex), pattern = "_"))
    pass.ex <- as.factor(sm.ex[3,])
    names(pass.ex) <- colnames(d.hg.ex)
    NXIDs.ex <- (sm.ex[2,])
    names(NXIDs.ex) <- colnames(d.hg.ex)
}
...
# Axis limits
#xmax <- max(es.hg.pdx$meanES, es.mm.pdx$meanES)
#ymax <- max(es.hg.cmb$meanES, es.mm.cmb$meanES)
#xmin <- min(es.hg.pdx$meanES, es.mm.pdx$meanES)
#ymin <- min(es.hg.cmb$meanES, es.mm.cmb$meanES)
...
test <- colnames(cont.mat) # test: KIRC vs the other types
# Cancer
...
g.deg.hg <- ggplot(sig.m, aes(x = logFC.x, y = logFC.y)) + 
    ...
    geom_smooth(method=lm, color = "red", fill = "red", size = .3, fullrange=TRUE) +
    annotate("text", x = -2.5, y = 10, fontface="italic", size=2.5,
            label = paste("r =", round(cor.eff, digit = 3))) + 
...
g.deg.mm <- ggplot(sig.m, aes(x = logFC.x, y = logFC.y)) + 
    ...
    geom_point(size = 0.5, alpha = 0.5) + 
    geom_smooth(method=lm, color = "red", fill = "red", size = .3, fullrange=TRUE) +
    annotate("text", x = -1, y = 7, fontface="italic", size=2.5,
            label = paste("r =", round(cor.eff, digit = 3))) + 
    xlab("logFC, Original data") + ylab("logFC, Original+External data") +
...
#library(gplots)
#ls <- list(DEGs_PDX = d.de.m$Gene[d.de.m$adj.P.Val.x<0.05],
#          DEGs_PDX.EGA = d.de.m$Gene[d.de.m$adj.P.Val.y<0.05])
#venn(ls)
...
# Load expression data in TPM values of External KIRC PDXs (EGAD00001003895)
load("../../data/External_PDX/EGAD00001003895/gene.exp.hg.EGAD00001003895.rda")
d.hg.ex1 <- gene.exp.hg.EGAD00001003895$abundance # TPM values
# annotation
sm <- read_tsv("../../data/External_PDX/EGAD00001003895/samples_KIRC.TG.tsv") #%>% column_to_rownames("GeneSymbol")
ids <- c()
for (egf in colnames(d.hg.ex1)){
...
sm.ex1 <- data.frame(str_split(ids, pattern = "_"))
pass.ex1 <- gsub("[a-c]$", "", gsub("TG", "", sm.ex1[2,]))
NXIDs.ex1 <- sm.ex1[1,]
colnames(d.hg.ex1) <- paste("PDX_ExternalPDX_KIRC", NXIDs.ex1, pass.ex1, sep = "_")

# Merge Original PDX & External PDX -> Combined PDX data
data.frame(d.hg) %>% rownames_to_column("Gene") -> d1
data.frame(d.hg.ex) %>% rownames_to_column("Gene") -> d2
data.frame(d.hg.ex1) %>% rownames_to_column("Gene") -> d3
merge(d1, d2, by = "Gene") -> d.tmp
merge(d.tmp, d3, by = "Gene") -> d.hg.cmb
...
d.ccle[ ,grep("KIRC", colnames(d.ccle))] %>% tibble::rownames_to_column("Gene") -> d.ccle
...
g.box <- cowplot::plot_grid(p1.sig, p2.sig, p3.sig, nrow = 1, rel_widths = c(1,1,1))
g.box
...
p
ggsave("./FigS3_Ori-Ex.jpg", dpi = 500, width = 200, height = 200, units = "mm")
